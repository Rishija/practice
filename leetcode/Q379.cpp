...
public:
    ...
    PhoneDirectory(int maxNumbers) : A(maxNumbers), index(0), max(maxNumbers) {
    ...
    void release(int number) {
        if(map.find(number) == map.end())
            return;
        int numIndex = map[number];
        map.erase(number);
        --index;
        if(numIndex == index) return;
        swap(A[numIndex], A[index]);
        map[A[numIndex]] = numIndex;
    }
};
